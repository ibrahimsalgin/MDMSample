...
public class MDMService extends Service {
    ...
    public void openYoutube(String videoId) {
        ...
        youtubeIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(getString(R.string.youtube_uri) + videoId));
        ...
        if (this.getPackageManager().queryIntentActivities(youtubeIntent, PackageManager.MATCH_DEFAULT_ONLY).size() == 0) {
            Toast.makeText(this, R.string.install_youtube_warning, Toast.LENGTH_SHORT);
        } else {
            youtubeRunnable.run();
        }
    }
    ...
    private Runnable cameraRunnable = new Runnable() {
        ...
        public void run() {
            try {
                ...
                Toast.makeText(MDMService.this, R.string.camera_state_change_warning, Toast.LENGTH_SHORT).show();
            }
        }
    };
